...
##
# Contains utilities for remote instances using CWT.
#
# Lazy subject-wide include for remote_cwt/db — not on CWT_INC.
# Loaded by bootstrap phase 90 when any action in this subject dir sources
# cwt/bootstrap.sh. Do not restore as extension-root remote_cwt.inc.sh.
#
...
u_remote_sync_db_from() {
  ...
  if [[ -n "$p_option" ]]; then
    ...
    case "$p_option" in new)
      rsf_remote_dump_file="$(cwt/extensions/remote_cwt/remote/exec.sh "$p_id" "cwt/extensions/db/db/get_dump.sh new")"
      rsf_remote_dump_file="${rsf_remote_dump_file#$REMOTE_INSTANCE_DOCROOT/}"
    esac
  else
    rsf_remote_dump_file="$(cwt/extensions/remote_cwt/remote/exec.sh "$p_id" "cwt/extensions/db/db/get_dump.sh")"
    rsf_remote_dump_file="${rsf_remote_dump_file#$REMOTE_INSTANCE_DOCROOT/}"
  fi
